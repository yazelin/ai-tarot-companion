...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys().then(keys =>
      // 只清自己的 tarot-*:CacheStorage 是 per-origin,yazelin.github.io 所有專案共用同一份,
      // 無差別刪會把 gewu、neko 等別站的離線包整包清掉,而且毫無徵兆。
      Promise.all(keys.filter(k => k.startsWith('tarot-') && k !== CACHE).map(k => caches.delete(k)))
